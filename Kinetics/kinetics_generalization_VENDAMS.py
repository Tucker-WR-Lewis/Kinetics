...
def getodes(kinin_temp,a,b):
    global reactions, kD_fun
    f = open(kinin_temp)
    text = f.read()
    text_out = text.split()
    f.close()
...
def batch_import(species_temp, files_temp, iso_temp):
    ...
    a = float(string_list[10][0])
    b = float(string_list[11][0])
    ...
    return rxntime_temp, neutral_reactant_temp, data_temp.transpose(), neutral_con_temp, initial_cons_temp, a, b
...
def getgof(params,numpoints_temp,numks,ydatas,neutral_con_temp, iso_temp, k_l_bounds_temp):
    global num_analyze, res_per_square, numk
    numk = numks
    final_res = []
...
def solve(y_0,*ki):
    ...
    # a = a_list[num_analyze]
    # b = b_list[num_analyze]
    # kD = a*y_0[1]**b
    y = y_0
    t = 0
    h = rxntime[num_analyze]/10000000
    if len(ki) == 1:
        ki = ki[0]
    ki = np.array(ki)
    # ki[-1] = kD
...
def initial_fitting(params_bounds_temp, gofargs_temp, nonlincon, kinin_temp, files_temp, rois_temp):
    global constraints_new, rxntime, f_jit, a_list, b_list, numk, a ,b
    ydot, y, k, k_l_bounds, k_h_bounds, species_0, constraints_new, con_limits_low, con_limits_high, names, reactmap, prodmap, iso_index, mass_descrim = getodes(kinin_temp,a,b)
    t = sym.symbols('t')
    ...
    for input_file in files_temp:
        if '.BATCHEXP' or '.batchexp' in input_file:
            rxntimes, neutral_reactants, datas, neutral_cons, initial_conss, a, b = batch_import(names, input_file, iso_index)
            a_list.append(a)
            b_list.append(b)
    ...
    res = sp.optimize.differential_evolution(getgof, params_bounds_temp, args = gofargs_temp, strategy='best2bin', 
                                              maxiter=2000, popsize=1, tol=0.1, mutation= (0.01, 1.99), recombination=0.90, 
...
def error_analysis(best_fit, gofargs_temp, param_bounds_temp, numsims_temp, names_temp, nonlincon_temp, kinin_temp,numcpus_temp, files_temp, rois_temp):
    ...
    for num_analyze in range(len(fake_data_temp)):
        ...
        totals = np.sum(fit_datas,axis = 1)
        norm = (fit_datas.transpose()/totals).transpose()
        residual = (norm - fake_data_temp[num_analyze])/(fake_data_temp[num_analyze]+0.1)
...
def sim_monte(fit_stdev, fit_data, best_fit, sim_gofargs, param_bounds_temp, nonlincon_temp, kinin_temp, files_temp, rois_temp):
    global constraints_new, rxntime, f_jit, numk, a_list, b_list, a, b
    #unpacking gofargs
    fake_data_temp = sim_gofargs[2]
    a = 13.757
    b = 0.133 
    ydot, y, k, k_l_bounds, k_h_bounds, species_0, constraints_new, con_limits_low, con_limits_high, names, reactmap, prodmap, iso_index, mass_descrim = getodes(kinin,a,b)
    t = sym.symbols('t')
    f_lamb = sym.lambdify((t, y) + k, ydot, "numpy")
    f_jit = nb.njit(f_lamb)
    numk = len(k)
    ...
    for nums in range(len(fit_stdev)):
        ...
        # totals = np.sum(sim_datas,axis=1)
        # sim_datas = (sim_datas.transpose()/totals).transpose()
...
def get_all_inputs(kinin_temp, batchin_temp, BLS_temp):
    ...
    # k_l_bounds = np.concatenate((k_l_bounds,[0.001]))
    # k_h_bounds = np.concatenate((k_h_bounds,[0.01]))
    ...
    return [param_bounds, gofargs, nonlincon, files_grouped, initial_cons, species_0, f_jit, rxntime, k, names, ydot, constraints_new, k_h_bounds, a_list, b_list]
...
a_list = inputs_tuple[13]
b_list = inputs_tuple[14]
a = a_list[0]
b = b_list[0]
...
if __name__ == '__main__':
    ...
    for input_files in files_grouped:
        ...
        for i in range(len(data)):
            ...
            plt.semilogy(neutral_con[i][sorting_index],norm[sorting_index])
        ...
        for index in range(len(names)):
            ...
            if np.any(full_sim_data[0][0][:,index]):
                for plotting_params in sim_params:
                    ...
                    norm = (sim_data.transpose()/totals).transpose()
                    norm = norm[:,index]
                    plt.semilogy(neutral_con[0],norm,'o',color = 'red', alpha = 0.05)
