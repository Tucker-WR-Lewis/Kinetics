...
gofs_sorted_indices = np.argsort(gofs[0])
indices = gofs_sorted_indices[0:int(np.floor(gofs[0].shape[0]*0.75))]
...
# Plots =     (['kT','all'],)
Plots = (['kT',['k13','k14','k15','k16','k17'],['k12'],['k11'],['k26']],)
Plots = (['kT',['k7','k8'],['k6'],['k5'],['k9','k10']],)
# Plots =     (['kT','Ta+'],
#               ['kT','Ta(CH2)+'])
...
for plot in Plots:
    ...
    temp_data = []
    legends = []
    legends = [np.array(['N4+ + O', 'N4+ + N', 'N4+ + NO', 'N4+ + O2'])]
    legends = [np.array(['N3+ + O', 'N3+ + N', 'N3+ + NO', 'N3+ + O2'])]
    if plot[0] == 'ks':
        if plot[1] == 'all':
    ...
    if plot[0] == 'kT':
        ...
        if type(plot[1]) == list:
            kt_data = []
            plot[0] = 'ks'
            for constants_to_sum in plot[1:]:
                kt_indices_temp = []
                for constant in constants_to_sum:
                    kt_indices_temp.append(list(ks).index(constant))
                if np.any(np.sum(data[:,:,kt_indices_temp],axis = 2)):
                    titles.append(plot[1])
                    kt_data.append(sum_plot_data(data,kt_indices_temp,indices))
            temp_data.append(kt_data)
